...
public class RemoveBuilderCommandExecutor implements CommandExecutor
{
	private HomeManager plugin;
	
	public RemoveBuilderCommandExecutor(HomeManager plugin)
	{
		this.plugin = plugin;
	}
			...
			if (args.length == 2)
			{
				...
				if (tempPlayer != null)
				{
					...
					{		
						...
						{
							plugin.setRegion(x, plugin.getRegions().get(x).removeBuilder(args[1]));
							sender.sendMessage(ChatColor.GREEN + args[1] + " is no longer allowed to build in home " + args[0]);
							tempPlayer.sendMessage(ChatColor.RED + "You are no longer allowed to build in home " + args[0]);
							return true;
						}
						else
						{
							sender.sendMessage(ChatColor.RED + "You must be the home owner to remove a builder.");
							return true;
						}
					}
					else
